...
impl SnapshotService {
    ...
    async fn calculate_holdings_snapshots_internal(
        ...
        debug!(
            "Starting snapshot calculation (Holdings Only) for {:?} accounts. Force full: {}. Offset: {}",
            account_ids_param, force_full_calculation, timezone_offset_minutes
        );

        let (accounts_to_process, all_activities, min_activity_date, calculation_end_date) =
            self.fetch_required_data(account_ids_param, timezone_offset_minutes)?;
    ...
    fn fetch_required_data(
        ...
        // compute min_activity_date & calculation_end_date in local time
...
impl SnapshotServiceTrait for SnapshotService {
    async fn calculate_holdings_snapshots(
        ...
        self.calculate_holdings_snapshots_internal(account_ids, false, timezone_offset_minutes)
            .await
    }
    ...
    async fn force_recalculate_holdings_snapshots(
        ...
        self.calculate_holdings_snapshots_internal(account_ids, true, timezone_offset_minutes)
            .await
    }
